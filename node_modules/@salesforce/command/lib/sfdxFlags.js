...
function buildFilepath(options) {
    return option('filepath', options, (val) => {
        return Promise.resolve(validateValue(core_2.sfdc.validatePathDoesNotContainInvalidChars(val), val, 'filepath'));
    });
}
function buildDirectory(options) {
    return option('directory', options, (val) => {
        return Promise.resolve(validateValue(core_2.sfdc.validatePathDoesNotContainInvalidChars(val), val, 'directory'));
    });
}
...
function buildEmail(options) {
    return option('email', options, (val) => {
        return Promise.resolve(validateValue(core_2.sfdc.validateEmail(val), val, 'email'));
    });
}
function buildId(options) {
    return option('id', options, (val) => {
        return Promise.resolve(validateValue(core_2.sfdc.validateSalesforceId(val), val, 'id', ` ${messages.getMessage('error.FormattingMessageId')}`));
    });
}
